...
function makeGroup(overrides: Partial<PatternGroup> = {}): PatternGroup {
  return {
    fingerprint: 'fp-1',
    frames: [
      { id: 'f1', name: 'Login Screen', width: 375, height: 812, childCount: 2, maxDepth: 4, componentIds: [], componentNames: [], aspectRatio: 0.46, layoutMode: 'VERTICAL', cornerRadius: 8, hasAutoLayout: true, fillCount: 1, childTypeDistribution: {} },
      { id: 'f2', name: 'Register Screen', width: 375, height: 812, childCount: 3, maxDepth: 4, componentIds: [], componentNames: [], aspectRatio: 0.46, layoutMode: 'VERTICAL', cornerRadius: 8, hasAutoLayout: true, fillCount: 1, childTypeDistribution: {}, fileKey: 'abc', fileName: 'Auth Flows' },
    ],
    consistency: 90,
    componentUsage: [],
    nameMatches: [],
...
describe('FrameDetailPanel', () => {
  ...
  it('shows consistency badge when group provided', () => {
    render(
      <FrameDetailPanel frame={makeFrame()} group={makeGroup()} onBack={vi.fn()} onOpenInFigma={vi.fn()} />,
    );
  ...
  it('uses frameName prop when frame is null', () => {
    render(
      <FrameDetailPanel frame={null} group={null} frameName="Fallback Name" onBack={vi.fn()} onOpenInFigma={vi.fn()} />,
    );
  ...
  it('shows "None" for zero corner radius', () => {
    render(
      <FrameDetailPanel frame={makeFrame({ cornerRadius: 0 })} group={null} onBack={vi.fn()} onOpenInFigma={vi.fn()} />,
    );
  ...
  it('formats mixed corner radius array', () => {
    render(
      <FrameDetailPanel frame={makeFrame({ cornerRadius: [8, 8, 0, 0] })} group={null} onBack={vi.fn()} onOpenInFigma={vi.fn()} />,
    );
