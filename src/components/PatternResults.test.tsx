...
function makeGroup(overrides: Partial<PatternGroup> = {}): PatternGroup {
  return {
    ...
    frames: [
      {
        id: 'f1', name: 'Home', width: 375, height: 812,
        childCount: 5, maxDepth: 3, componentIds: [], componentNames: ['Button'],
        aspectRatio: 0.46, layoutMode: 'VERTICAL', cornerRadius: 0,
        hasAutoLayout: true, fillCount: 1, childTypeDistribution: {},
      },
    ],
    consistency: 85,
    componentUsage: [],
...
describe('PatternResults', () => {
  ...
  it('renders frame buttons with name', () => {
    render(
      <PatternResults groups={[makeGroup()]} onFrameClick={vi.fn()} onOpenInFigma={vi.fn()} />,
    );
  ...
  it('shows consistency badge for groups with 2+ frames', () => {
    const group = makeGroup({
      frames: [
        { id: 'f1', name: 'A', width: 375, height: 812, childCount: 2, maxDepth: 2, componentIds: [], componentNames: [], aspectRatio: 0.46, layoutMode: 'VERTICAL', cornerRadius: 0, hasAutoLayout: true, fillCount: 1, childTypeDistribution: {} },
        { id: 'f2', name: 'B', width: 375, height: 812, childCount: 2, maxDepth: 2, componentIds: [], componentNames: [], aspectRatio: 0.46, layoutMode: 'VERTICAL', cornerRadius: 0, hasAutoLayout: true, fillCount: 1, childTypeDistribution: {} },
      ],
    });
  ...
  it('calls onFrameClick with correct args', () => {
    ...
    const group = makeGroup();
    render(
      <PatternResults groups={[group]} onFrameClick={onFrameClick} onOpenInFigma={vi.fn()} />,
    );
  ...
  it('shows file badge when frame has fileName', () => {
    const group = makeGroup({
      frames: [
        { id: 'f1', name: 'Card', width: 200, height: 300, childCount: 1, maxDepth: 1, componentIds: [], componentNames: [], aspectRatio: 0.67, layoutMode: 'NONE', cornerRadius: 0, hasAutoLayout: false, fillCount: 0, childTypeDistribution: {}, fileName: 'Design System' },
      ],
    });
    render(<PatternResults groups={[group]} onFrameClick={vi.fn()} onOpenInFigma={vi.fn()} />);
