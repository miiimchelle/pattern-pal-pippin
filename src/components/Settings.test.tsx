...
describe('Settings', () => {
  ...
  it('pre-fills values from props', () => {
    ...
    expect(
      screen.getByPlaceholderText(/Design-System/),
    ).toHaveValue('https://figma.com/file/a\nhttps://figma.com/file/b');
  });
  ...
  it('shows Testing... when isTestingConnection is true', () => {
    render(<Settings {...defaults} token="t" onTestConnection={vi.fn()} isTestingConnection={true} />);
  ...
  it('shows connection error message', () => {
    render(
      <Settings
        ...
        token="t"
        onTestConnection={vi.fn()}
        connectionTest={{ tokenValid: false, teamIdValid: false, userName: '', error: 'Network error' }}
      />,
    );
