...
function debugLog(location: string, message: string, data: Record<string, unknown>, hypothesisId: string): void {
  figma.ui.postMessage({ type: 'debug-log', payload: { location, message, data, hypothesisId, timestamp: Date.now() } });
}
// #endregion
...
function isPrimaryButton(node: InstanceNode): boolean {
  ...
  if (props && Object.keys(props).length > 0) {
    for (const key of Object.keys(props)) {
      ...
      if (
        typeof val === 'string' &&
        (val.toLowerCase() === 'primary' || val.toLowerCase() === 'default')
      ) {
        return true;
      }
    }
...
async function findButtonInstances(
  node: SceneNode,
  namesSeen: string[],
  ...
  if (node.type === 'INSTANCE') {
    ...
    const nameMatches =
      isButtonComponentName(componentName) || isButtonComponentName(instanceName);
    // #region agent log
    debugLog('code.ts:findButtonInstances', 'Instance node found', { instanceName, componentName, nameMatches, nodeType: node.type }, 'B,C');
    // #endregion
...
async function checkPrimaryButtonPerFrame(): Promise<{
  ...
  const pageChildTypesSummary = pageChildTypes
    .map((p) => `${p.type}:${p.name}`)
    .slice(0, 8);
  ...
  // #region agent log
  debugLog('code.ts:checkPrimaryButtonPerFrame', 'Check started', { containerCount: frames.length, containerNames: frames.slice(0, 10).map(f => f.name) }, 'B');
  // #endregion
  for (const frame of frames) {
    ...
    // #region agent log
    debugLog('code.ts:checkPrimaryButtonPerFrame:loop', 'Container checked', { frameName: frame.name, frameType: frame.type, totalButtonsInFrame: allButtonInstances.length, primaryButtonCount: primaryButtons.length, buttonNames: allButtonInstances.map(b => b.name), primaryNames: primaryButtons.map(b => b.name) }, 'B,D,E');
    // #endregion
...
figma.ui.onmessage = async (msg: { type: string; payload?: unknown }) => {
  ...
  switch (msg.type) {
    ...
    case 'highlight-frame': {
      const highlightId = (msg as { type: string; frameId?: string }).frameId;
      if (highlightId) {
        zoomToFrameId(highlightId);
      }
      break;
    }
